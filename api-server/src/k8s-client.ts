...
export class K8sClient {
  ...
  // Gate proxy configuration constants
  private readonly gateConfigMapName = 'gate-config';

  /**
   * Update Gate proxy routes to include all current Minecraft servers.
   * This enables dynamic routing to multiple servers with offline MOTD fallback.
   *
   * Each server gets a route based on its K8s resource name:
   * - Host pattern: "{server-name}.*" (e.g., "mc-490c4084faaa.*")
   * - Backend: "{server-name}.minecraft-servers.svc.cluster.local:25565"
   *
   * Routes are generated for ALL servers regardless of status - Gate handles
   * showing offline MOTD when the backend is unavailable.
   */
  async updateGateRoutes(): Promise<void> {
    if (!this.k8sAvailable) {
      console.warn('[K8sClient] K8s not available, skipping Gate route update');
      return;
    }

    try {
      // Get all servers to generate routes
      const servers = await this.listMinecraftServers();

      // Generate routes for each server (host-based for multi-server)
      ...
      // Add catch-all route for unknown hosts
      routes.push({
        host: '*',
        backend: 'localhost:25565', // Non-existent, will trigger fallback
        fallback: {
          motd: '§cNo server found\n§7Add server hostname to your hosts file',
          version: {
            name: '§7Unknown Host',
            protocol: -1,
          },
      ...
      // Build the Gate config YAML
      const configYaml = `# Gate Lite Mode - reverse proxy with fallback MOTD
# Auto-generated by API server - do not edit manually
config:
  bind: 0.0.0.0:25565
  lite:
    enabled: true
...
${routes
  .map(
    (route) => `      - host: "${route.host}"
        ...
        fallback:
          motd: |
            ${route.fallback.motd.split('\n').join('\n            ')}
          version:
            name: "${route.fallback.version.name}"
            protocol: ${route.fallback.version.protocol}`
  )
  .join('\n')}
`;
      ...
      try {
        // Try to replace existing ConfigMap
        await this.coreApi!.replaceNamespacedConfigMap({
          name: this.gateConfigMapName,
          namespace: this.namespace,
          body: configMap,
        });
        console.log(`[K8sClient] Updated Gate config with ${servers.length} server routes`);
      } catch (replaceError: any) {
        if (replaceError.response?.statusCode === 404) {
          // ConfigMap doesn't exist, create it
          await this.coreApi!.createNamespacedConfigMap({
            namespace: this.namespace,
            body: configMap,
          });
          console.log(`[K8sClient] Created Gate config with ${servers.length} server routes`);
        } else {
          throw replaceError;
        }
      }

      // Restart Gate deployment to pick up config changes
      // Gate doesn't support hot-reload, so we need to restart the pod
      await this.restartGateDeployment();
    } catch (error) {
      console.error('[K8sClient] Failed to update Gate routes:', error);
      throw error;
    }
  }

  /**
   * Restart the Gate deployment by deleting its pods.
   * The deployment controller will automatically recreate them with the new config.
   */
  private async restartGateDeployment(): Promise<void> {
    try {
      // List Gate pods by label selector
      const pods = await this.coreApi!.listNamespacedPod({
        namespace: this.namespace,
        labelSelector: 'app=gate',
      });

      // Delete each pod to trigger recreation
      for (const pod of pods.items) {
        if (pod.metadata?.name) {
          ...
          console.log(`[K8sClient] Deleted Gate pod: ${pod.metadata.name}`);
        }
      }

      if (pods.items.length > 0) {
        console.log('[K8sClient] Triggered Gate restart by deleting pods');
      } else {
        console.log('[K8sClient] No Gate pods found to restart');
      }
    } catch (error: any) {
      if (error.response?.statusCode === 404) {
        console.warn('[K8sClient] Gate deployment not found, skipping restart');
      } else {
        console.error('[K8sClient] Failed to restart Gate:', error.message);
        // Don't throw - route update succeeded, just restart failed
      }
    }
  }
}
